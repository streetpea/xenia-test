...
set -x
mkdir lucas/
pacman -Syu --noconfirm cmake git jack2 findutils ninja gcc base-devel gnupg lsb-release python-opencv wget lua gtk3 lz4 glew libx11 sdl2 pkgconf curl desktop-file-utils binutils make vulkan-icd-loader sdl2 vulkan-headers xcb-util-keysyms xkeyboard-config wayland libxrandr mesa libxinerama libxcursor binutils clang-tools-extra
...
python3 xenia-build setup --target_os=linux && python3 xb premake --cc clang && python3 xb build --config=release
#mkdir build2/
#cd ./build2/
